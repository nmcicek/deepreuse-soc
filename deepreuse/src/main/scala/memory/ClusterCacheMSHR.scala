...
class ClusterCacheMSHR(id: Int)(implicit val edge: TLEdgeOut, val p: Parameters) extends Module with ClusterCacheParameters {
  val io = IO(new Bundle {
    val req_pri_val    = Bool(INPUT)
    val req_pri_rdy    = Bool(OUTPUT)
    val req_sec_val    = Bool(INPUT)
    val req_sec_rdy    = Bool(OUTPUT)
    val req_bits       = new CCMSHRReqInternal().asInput
    ...
    val refill = new CCRefillReq().asOutput // Data is bypassed
    val meta_read = Decoupled(new CCMetaReadReq)
    val meta_write = Decoupled(new CCMetaWriteReq)
    val replay = Decoupled(new CCReplayInternal)
    val wb_req = Decoupled(new CCWritebackReq(edge.bundle))
    val probe_rdy = Bool(OUTPUT)
  })

  io.suggestName("ClusterCacheMSHRIO")
  ...
  val req = Reg(new CCMSHRReqInternal)
  ...
  val rpq = Module(new Queue(new CCReplayInternal, nRPQ))
  ...
  when (io.mem_grant.valid) { 
    if(DEBUG_PRINTF_CACHE) printf("CC mshr %d acked true\n", UInt(id))
    acked := true.B 
  }

  when (state === s_drain_rpq && !rpq.io.deq.valid) {
    if(DEBUG_PRINTF_CACHE) printf("CC mshr %d at drain finished\n", UInt(id))
    state := s_invalid
  }
  when (state === s_meta_write_resp) {
    if(DEBUG_PRINTF_CACHE) printf("CC mshr %d at drain stage\n", UInt(id))
  ...
  when (state === s_meta_write_req && io.meta_write.ready) {
    if(DEBUG_PRINTF_CACHE) printf("CC mshr %d at meta write req\n", UInt(id))
    state := s_meta_write_resp
  }
  when (state === s_refill_resp && refill_done) {
    if(DEBUG_PRINTF_CACHE) printf("CC mshr %d at refill resp\n", UInt(id))
    new_coh := coh_on_grant
    state := s_meta_write_req
  }
  when (io.mem_acquire.fire()) { // s_refill_req
    if(DEBUG_PRINTF_CACHE) printf("CC mshr %d at refill req\n", UInt(id))
    state := s_refill_resp
  }
  when (state === s_meta_clear && io.meta_write.ready) {
    if(DEBUG_PRINTF_CACHE) printf("CC mshr %d at meta clear\n", UInt(id))
    state := s_refill_req
  }
  when (state === s_wb_resp && io.wb_req.ready && acked) {
    if(DEBUG_PRINTF_CACHE) printf("CC mshr %d at wb resp\n", UInt(id))
    state := s_meta_clear
  }
  when (io.wb_req.fire()) { // s_wb_req
    if(DEBUG_PRINTF_CACHE) printf("CC mshr %d at wb req\n", UInt(id))
    state := s_wb_resp
  }
  when (io.req_sec_val && io.req_sec_rdy) { // s_wb_req, s_wb_resp, s_refill_req
    if(DEBUG_PRINTF_CACHE) printf("CC mshr %d at secondary req\n", UInt(id))
  ...
  when (io.req_pri_val && io.req_pri_rdy) {
    if(DEBUG_PRINTF_CACHE) printf("CC mshr %d at primary req\n", UInt(id))
    req := io.req_bits
    acked := false.B
