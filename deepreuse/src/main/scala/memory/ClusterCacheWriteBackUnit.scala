...
class ClusterCacheWritebackUnit(implicit val edge: TLEdgeOut, val p: Parameters) extends Module with ClusterCacheParameters {
  val io = IO(new Bundle {
    val req = Decoupled(new CCWritebackReq(edge.bundle)).flip
    val meta_read = Decoupled(new CCMetaReadReq)
    val data_req = Decoupled(new CCDataReadReq)
    val data_resp = Bits(INPUT, rowBits)
    val release = Decoupled(new TLBundleC(edge.bundle))
  })

  val req = Reg(new CCWritebackReq(edge.bundle))
