...
class IDCacheMSHR(id: Int)(implicit val edge: TLEdgeOut, val p: Parameters) extends Module with IDCacheParameters {
  ...
  val grow_param                                  = new_coh.onAccess(M_XWR)._2
  val coh_on_grant                                = new_coh.onGrant(M_XWR, io.mem_grant.bits.param)
  ...
  val (cmd_requires_second_acquire, is_hit_again, _, dirtier_coh, dirtier_cmd) = new_coh.onSecondaryAccess(M_XWR, M_XWR)
  ...
  when (io.req_pri_val && io.req_pri_rdy) {
    ...
    val (is_hit, _, coh_on_hit) = old_coh.onAccess(M_XWR)
