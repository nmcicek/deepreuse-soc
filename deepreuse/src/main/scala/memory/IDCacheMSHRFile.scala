...
class IDCacheMSHRFile(implicit val edge: TLEdgeOut, val p: Parameters) extends Module with IDCacheParameters {
  val io = IO(new Bundle {
    val req = Decoupled(new ICMSHRReq).flip
    ...
    val refill = new ICRefillReq().asOutput
    val meta_read = Decoupled(new ICMetaReadReq) //idx way_en tag
    val meta_write = Decoupled(new ICMetaWriteReq) //data
    val replay = Decoupled(new ICReplay)
  ...
  io.suggestName("IDCacheMSHRFileIO")
  ...
  val refillMux = Wire(Vec(nMSHRs, new ICRefillReq))
  val meta_read_arb = Module(new Arbiter(new ICMetaReadReq, nMSHRs))
  val meta_write_arb = Module(new Arbiter(new ICMetaWriteReq, nMSHRs))
  val wb_req_arb = Module(new Arbiter(new ICWritebackReq(edge.bundle), nMSHRs))
  val replay_arb = Module(new Arbiter(new ICReplayInternal, nMSHRs))
  ...
  val mshrs = (0 until nMSHRs) map { i =>
    val mshr = Module(new IDCacheMSHR(i))
