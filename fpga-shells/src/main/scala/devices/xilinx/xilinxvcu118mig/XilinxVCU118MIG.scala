...
import fpgashells.ip.xilinx.vcu118mig.{VCU118MIGIOClocksReset, VCU118MIGIODDR, vcu118mig}
...
class XilinxVCU118MIGIsland(c : XilinxMIGParams)(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    io.port.addn_ui_clkout1     := blackbox.io.addn_ui_clkout1
...
class XilinxVCU118MIG(c : XilinxMIGParams)(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
