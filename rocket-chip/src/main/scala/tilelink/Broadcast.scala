...
class TLBroadcast(lineBytes: Int, numTrackers: Int = 4, bufferless: Boolean = false)(implicit p: Parameters) extends LazyModule with RocketDebugConstants
{
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      if(DEBUG_PRINTF_BH){
        when(in.a.fire()){
          printf("BH tl_in_a_address: 0x%x tl_in_a_data: 0x%x tl_in_a_size: 0x%x tl_in_a_source: 0x%x\n",
                  in.a.bits.address, in.a.bits.data, in.a.bits.size, in.a.bits.source)
        }
        when(in.d.fire()){
          printf("BH tl_in_d_data: 0x%x tl_in_d_size: 0x%x tl_in_d_source: 0x%x\n",
                  in.d.bits.data, in.d.bits.size, in.d.bits.source)
        }      
        when(in.e.fire()){
          printf("BH tl_in_e_sink: 0x%x\n",
                  in.e.bits.sink)
        } 
        when(out.a.fire()){
          printf("BH tl_out_a_address: 0x%x tl_out_a_data: 0x%x tl_out_a_size: 0x%x tl_out_a_source: 0x%x\n",
                  out.a.bits.address, out.a.bits.data, out.a.bits.size, out.a.bits.source)
        }
        when(out.d.fire()){
          printf("BH tl_out_d_data: 0x%x tl_out_d_size: 0x%x tl_out_d_source: 0x%x\n",
                  out.d.bits.data, out.d.bits.size, out.d.bits.source)
        }
      }
