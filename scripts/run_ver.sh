...
do	
	...
	do							
		...
		OtherInputs="\ \ tile->lshIO_conf_hashSize = ${layerHashSizes[i]};\n"
		OtherInputs="${OtherInputs}\ \ tile->lshIO_conf_vectorDim = ${layerVectorDim[i]};"
		sed -i '/^  tile->lshIO_conf_hashSize/d' deepreuse/src/main/resources/csrc/LSHUInt8Emulator.cc
		sed -i '/^  tile->lshIO_conf_vectorDim/d' deepreuse/src/main/resources/csrc/LSHUInt8Emulator.cc
		sed -i "/\/\/ other inputs/a ${OtherInputs}" deepreuse/src/main/resources/csrc/LSHUInt8Emulator.cc

		bin=${PWD}/sw/bin/cifarnet/int8/LSH_layer${i}_subvector${j}.bin
		make clean sim CONFIG="$1"LSH"$2"SimConfig bin=$bin -j4
