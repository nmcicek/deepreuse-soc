...
do
	echo "Running for hash size of "${layerHashSizes[i]}" and vector dimension of "${layerVectorDim[i]}
	...
	builds/lsh-accelerator/simulator-FPGA-FPGAChip 2> output_sim/$1_layer${i}.log
	grepped_value="$(grep -rni "completed" output_sim_basic/$1_layer${i}.log)"
	IFS=' ' read -r -a array <<< "$grepped_value"
	echo layer_num: ${i} cycle: ${grepped_value} >> output_sim_basic/$1.out
done
