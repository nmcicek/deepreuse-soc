...
case object UartKey extends Field[UARTParams]

case class UARTParams(
  address: Option[BigInt],
  dataBits: Int = 8,
  stopBits: Int = 1,
  divisorInit: Int = (BigInt(50000000)/115200).toInt,
  divisorBits: Int = 16,
  oversample: Int = 4,
  nSamples: Int = 3,
  nTxEntries: Int = 8,
...
trait HasUARTParameters {
  ...
  val uartParams = p(UartKey)
  def uartDataBits = uartParams.dataBits
}
...
class UARTModule(val c: UARTParams) extends Module
{
  ...
  require(c.divisorInit != 0, "UART divisor wasn't initialized during instantiation")
  require(c.divisorInit >> c.divisorBits == 0, s"UART divisor reg (width $c.divisorBits) not wide enough to hold $c.divisorInit")
  ...
  val div = Reg(init = UInt(c.divisorInit, c.divisorBits))
