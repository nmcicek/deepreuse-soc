...
class UartWrapperModule(outer: UartWrapper) extends BaseWrapperModule(outer) 
{
	...
	val uartModule = Module(new UARTModule(p(PeripheryUARTKey).head))
      ...
      when(txHeader === Bits(1)) {
        when(lshModule.io.centroidsOut.valid){
          if(DEBUG_PRINTF_UART){
            ...
            for(i<-0 until maxVectorDim)
              printf("---txHeader--- lshModule.io.centroidsOut.bits(%d): 0x%x\n", i.U, lshModule.io.centroidsOut.bits(i))
          }
          state := s_txClusterOut
          rClusterOut := lshModule.io.centroidsOut.bits
        }
    ...
    // Fetch Buffer
    val fetchBuffer = Module(new Queue(new LSHReq, 10, false, true))
    ...
    lshModule.io.req <> fetchBuffer.io.deq
    if(DEBUG_PRINTF_UART){
    ...
    if(DEBUG_PRINTF_UART){
      when(lshModule.io.req.fire()){
        printf("req fired -> InputID: 0x%x\n", lshModule.io.req.bits.inputID)
        for(i<-0 until maxVectorDim)
          printf("InputData(%d): 0x%x\n",i.U, lshModule.io.req.bits.inputData(i))
      }
    }
    // Connection with hashTable input of LSH module
    lshModule.io.hashTable.valid := hashTableValid
    for (i <- 0 until hashTableMem.length.toInt) {
      lshModule.io.hashTable.bits(i) := hashTableMem(i)
    }
    ...
    lshModule.io.clusterNum := Mux(state === s_rxHashConf, maxHashSize.U, accHashConfReg)
    lshModule.io.kernelDim := Mux(state === s_rxDimConf, maxVectorDim.U, accDimConfReg)
    // clearMems
